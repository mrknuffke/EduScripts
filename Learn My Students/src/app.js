...
(function () {
  ...
  function getEffectiveName(card) {
    if (!card) return '';
    const roster = (card.name || '').trim();
    const pref = (card.preferredName || '').trim();

    if (!pref) {
      return roster || 'Unknown Student';
    }

    if (!roster || roster === 'Unknown Student') {
      return pref;
    }

    // If preferred name already contains spaces (full name provided), use it directly
    if (pref.includes(' ')) {
      return pref;
    }

    const rosterTokens = roster.split(/\s+/).filter(Boolean);
    if (rosterTokens.length <= 1) {
      return pref;
    }

    // If preferred name matches one of the given/middle name tokens after the first token (e.g. "Yip Han Wong" with pref "Han")
    const lowerPref = pref.toLowerCase();
    const matchIdx = rosterTokens.slice(1).findIndex(t => t.toLowerCase() === lowerPref);
    if (matchIdx >= 0) {
      const remainingTokens = rosterTokens.slice(matchIdx + 2);
      const lastName = remainingTokens.length > 0 ? remainingTokens.join(' ') : rosterTokens[rosterTokens.length - 1];
      return `${pref} ${lastName}`;
    }

    // Standard case: replace first given name with preferred name
    // E.g. "Abigail Wang" -> "Abby Wang", "Sebastian Hehir" -> "Seb Hehir"
    const lastName = rosterTokens.slice(1).join(' ');
    return `${pref} ${lastName}`;
  }
  ...
  function gradeAnswer(inputStr, card) {
    const effectiveName = getEffectiveName(card);
    const normInput = normalizeString(inputStr);
    if (!normInput) return { outcome: 'miss', targetSpelling: effectiveName };

    const normEffectiveFull = normalizeString(effectiveName);
    const normRosterFull = normalizeString(card.name);
    const normPref = card.preferredName ? normalizeString(card.preferredName) : '';

    // 1. Full name match check (against effective full name, roster full name, or preferredName)
    const distEffective = levenshtein(normInput, normEffectiveFull);
    const distRoster = levenshtein(normInput, normRosterFull);
    const distPref = normPref ? levenshtein(normInput, normPref) : Infinity;

    if (
      distEffective <= allowedEdits(normEffectiveFull.length) ||
      distRoster <= allowedEdits(normRosterFull.length) ||
      (normPref && distPref <= allowedEdits(normPref.length))
    ) {
      return { outcome: 'correct', targetSpelling: effectiveName };
    }

    // 2. Single token match check
    const tokens = normEffectiveFull.split(' ')
      .concat(normRosterFull.split(' '))
      .concat(normPref ? normPref.split(' ') : []);
    ...
    for (const token of uniqueTokens) {
      ...
      if (distToken <= allowedEdits(token.length)) {
        return { outcome: 'partial', targetSpelling: effectiveName };
      }
    }
    ...
    return { outcome: 'miss', targetSpelling: effectiveName };
  }
    ...
    importData.cards.forEach((card, idx) => {
      ...
      cardEl.innerHTML = `
        ...
        <div class="import-card-pref-wrapper">
          <input type="text" class="input-text sm import-preferred-input" data-idx="${idx}" placeholder="Preferred Name (e.g. Abby)" value="${escapeHtml(card.preferredName || '')}">
        </div>
  ...
  function updateStudentNameDatalist(decks) {
    ...
    if (activeSession && activeSession.queue && activeSession.currentIndex < activeSession.queue.length) {
      ...
      remainingItems.forEach(item => {
        const c = item.card;
        const effective = getEffectiveName(c);
        if (effective && effective !== 'Unknown Student') {
          names.add(effective);
        }
      });
    } else if (decks) {
  ...
  function handleRevealAnswer() {
    ...
    const feedbackEl = document.getElementById('study-feedback-area');
    feedbackEl.innerHTML = `<span class="feedback-badge missed">✗ Missed</span> Correct Answer: <span class="feedback-target">${escapeHtml(getEffectiveName(currentItem.card))}</span>`;
      ...
      activeSession.missedCards.forEach((item) => {
        ...
        rowEl.innerHTML = `
          <div class="missed-card-info">
            ...
            <span class="missed-card-name">${escapeHtml(getEffectiveName(item.card))}</span>
          </div>
  ...
  function renderClassGrid() {
    ...
    activeGridDeck.cards.forEach(card => {
      ...
      if (isEditing) {
        cardEl.innerHTML = `
          <img src="${card.photo}" class="grid-student-photo" alt="Photo of ${escapeHtml(getEffectiveName(card))}">
          <div class="grid-student-edit-box">
            <div class="grid-edit-roster-name" title="Legal: ${escapeHtml(card.name)}">Legal: ${escapeHtml(card.name)}</div>
            <input type="text" class="input-text sm grid-pref-input" placeholder="Preferred Name (e.g. Abby)" value="${escapeHtml(card.preferredName || '')}">
        ...
        const effectiveName = getEffectiveName(card);
        const hasPref = card.preferredName && card.preferredName.trim().length > 0 && effectiveName !== card.name;
        const legalSub = hasPref ? `<div class="grid-student-pref-name">Legal: ${escapeHtml(card.name)}</div>` : '';
        cardEl.innerHTML = `
          <img src="${card.photo}" class="grid-student-photo" alt="Photo of ${escapeHtml(effectiveName)}">
          <div class="grid-student-name-wrapper ${isHidden ? 'hidden-name' : ''}">
            <div class="grid-student-name">${escapeHtml(effectiveName)}</div>
            ${legalSub}
