...
-   **AP Biology Lab Standard**: A lab below **3** counts as outstanding work; a lab at 3 (`Meeting`) does not, since reaching 4 is what the lab standard note asks for rather than a problem to flag. AP Bio reports containing labs carry a note stating that every lab must finish the semester at 4 / `Meeting with Distinction` to support a full grade claim for standards-based work, along with how many labs on that report are not there yet and a reminder that the scores are not final until the end of the semester.
